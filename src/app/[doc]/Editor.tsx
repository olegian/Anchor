...
export function Editor() {
  ...
  const editor = useEditor({
    extensions: [
      ...
      AnnotationMagic<ContextSelection>().configure({
        onAnnotationListChange: (annotations: Annotation<ContextSelection>[]) => {
            // on context creation / deletion / mutation
            console.log(`Annotation List Changed: ${annotations}`)
            editor.chain().focus().run()
        },
        onSelectionChange: (selectedAnnotations: Annotation<ContextSelection>[]) => {
            // on context creation / deletion / mutation
  ...
  const addAnnotationHandler = () => {
    editor.chain().focus().addAnnotation({
        label: "test"
    }).run()
  }
