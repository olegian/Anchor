...
// New function to regenerate the last response
export async function regenerateResponse(
  ...
  try {
    ...
    // Get conversation history and remove the last response to regenerate
    if (!conversationHistory.has(handleId)) {
      conversationHistory.set(handleId, []);
    }
    const history = conversationHistory.get(handleId)!;
    
    // Remove the last response from history if it exists
    if (history.length > 0 && history[history.length - 1].role === "model") {
      history.pop();
    }
    
    // Also remove the last user message to regenerate properly
    if (history.length > 0 && history[history.length - 1].role === "user") {
      history.pop();
    }

    // Generate new response
    const text = await generateLLMResponse(userPrompt, documentContext, history, env);
    ...
    await liveblocks.mutateStorage(docId, ({ root }) => {
      ...
      if (exchanges && exchanges.length > 0) {
        exchanges.get(exchanges.length - 1)?.set("response", text);
        // Update timestamp to indicate regeneration, something to cinsider...
